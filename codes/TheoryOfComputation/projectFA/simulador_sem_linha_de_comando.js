...
const automatoJSON = fs.readFileSync('transition.aut', 'utf-8');
...
const entradasCSV = fs.readFileSync('entradas2.in', 'utf-8');
...
function processarEntradas(automato, entradas) {
    ...
    for (const entrada of entradas) {
        ...
        for (const simboloAtual of entrada) {
            ...
            const transicoesNormais = transicoes.find((transicao) => transicao.read === simboloAtual);
            ...
            if (transicoesNormais || transicoesVazias) {
                // Atualiza o estado atual com base nas transições encontradas
                estadoAtual = (transicoesNormais && transicoesNormais.to) || (transicoesVazias && transicoesVazias.to);

                // Se o estado atual for indefinido, a cadeia foi rejeitada
                if (estadoAtual === undefined) {
                    resultados.push({ entrada: entradasCSV, resultadoEsperado, resultadoObtido: '0', tempoExecucao: calcularTempoExecucao(inicioTempo) });
                    break;
                }
            } else {
        ...
        if (estadoAtual !== undefined && automato.estadoFinal.includes(estadoAtual)) {
            resultados.push({ entrada: entradasCSV, resultadoEsperado, resultadoObtido: '1', tempoExecucao: calcularTempoExecucao(inicioTempo) });
            ...
            resultados.push({ entrada: entradasCSV, resultadoEsperado, resultadoObtido: '0', tempoExecucao: 0 });
        }
    }
